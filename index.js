        ...
        visitor: {
            ...
            AssignmentExpression(path) {
                ...
                if (t.isMemberExpression(lhs)) {
                    ...
                    path.replaceWith(t.callExpression(moriMethod("assoc"), [lhs.object, lhs.property, rhs]));
                }
            },
            MemberExpression(path) {
                if (path.node.isClean) return;
                if (t.isAssignmentExpression(path.parent)) return;
                if (path.node.object.name == "console" || path.node.object.name == "window" || path.node.object.name == "global") return;
